      ...
      if (editingActivityId) {
        ...
        if (repeatScheduleConfig) {
          ...
          if (editingActivity?.scheduleGroupId) {
            const q = query(collection(db, 'schoolActivities', familyId, 'activities'), where('familyId', '==', familyId), where('scheduleGroupId', '==', editingActivity.scheduleGroupId));
            ...
            for (const d of snapshot.docs) {
              ...
              if (dateField) {
                ...
                if (!repeatScheduleConfig.days.includes(evtDate.getDay())) {
                  await deleteDoc(doc(db, 'schoolActivities', familyId, 'activities', d.id));
                  deletedCount++;
                }
              }
            }
        ...
        for (let w = 0; w < repeatScheduleConfig.weeks; w++) {
          ...
          for (let d = 0; d < 7; d++) {
            ...
            if (repeatScheduleConfig.days.includes(date.getDay())) {
              ...
              await addDoc(collection(db, 'schoolActivities', familyId, 'activities'), {
                ...activityData,
                dateFrom: dateStr,
                dateTo: dateStr,
                    ...
                    <View style={styles.field}>
                      ...
                      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                        {REMINDER_OPTIONS.map((option) => (
                          <TouchableOpacity key={option.value} style={[styles.personChip, { backgroundColor: activityForm.reminder === option.value ? SCHOOL_THEME : colors.inputBackground }]} onPress={() => setActivityForm(f => ({ ...f, reminder: option.value }))}>
                          ...
                          if (editingActivityId) {
                            ...
                            if (editingActivity?.scheduleGroupId) {
                              try {
                                ...
                                const q = query(collection(db, 'schoolActivities', familyId, 'activities'), where('familyId', '==', familyId), where('scheduleGroupId', '==', groupId));
                                const snapshot = await getDocs(q);
