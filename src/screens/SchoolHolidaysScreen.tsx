...
import { getSchoolHolidays, addSchoolHoliday, updateSchoolHoliday, deleteSchoolHoliday } from '../services/schoolService';
...
export const SchoolHolidaysScreen: React.FC<Props> = ({ navigation, route }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId) { await updateSchoolHoliday(editingId, data); }
      else { await addSchoolHoliday(data); }
  ...
  const handleSaveUrlImport = async () => {
    ...
    try {
      for (const h of selected) {
        await addSchoolHoliday({ title: h.title, dateFrom: h.dateFrom, dateTo: h.dateTo, timeFrom: h.timeFrom || '', timeTo: h.timeTo || '', yearId: selectedYear.id, childId: child.id, familyId });
      }
  ...
  const renderHoliday = (h: SchoolHoliday) => (
    <TouchableOpacity key={h.id} style={[styles.holidayCard, { backgroundColor: colors.surface, borderLeftWidth: 3, borderLeftColor: SCHOOL_THEME }]} onPress={() => { setEditingId(h.id); setForm({ title: h.title, dateFrom: h.dateFrom, dateTo: h.dateTo || '', timeFrom: h.timeFrom || '', timeTo: h.timeTo || '' }); setShowAddModal(true); }} onLongPress={() => setActionModal({ visible: true, id: h.id, title: h.title })}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
        <View style={{ width: 28, height: 28, borderRadius: 14, backgroundColor: SCHOOL_THEME + '20', alignItems: 'center', justifyContent: 'center' }}>
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top']}>
      ...
      {/* Add button */}
      <TouchableOpacity style={[styles.fab, { backgroundColor: SCHOOL_THEME }]} onPress={() => { setEditingId(null); setForm({ title: '', dateFrom: '', dateTo: '', timeFrom: '', timeTo: '' }); setShowAddModal(true); }}>
      ...
      <Modal visible={showAddModal} transparent animationType="fade">
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setShowAddModal(false)}>
          <TouchableOpacity activeOpacity={1} style={[styles.modalContent, { backgroundColor: colors.surface }]}>
            ...
            <TextInput style={[styles.modalInput, { backgroundColor: colors.inputBackground, color: colors.text }]} value={form.dateFrom} onChangeText={(v) => setForm(f => ({ ...f, dateFrom: v }))} placeholder={t('school.placeholderDateFrom')} placeholderTextColor={colors.textDisabled} />
            <TextInput style={[styles.modalInput, { backgroundColor: colors.inputBackground, color: colors.text }]} value={form.dateTo} onChangeText={(v) => setForm(f => ({ ...f, dateTo: v }))} placeholder={t('school.placeholderDateTo')} placeholderTextColor={colors.textDisabled} />
                ...
                <>
                  ...
                  {urlResults.map((h, i) => (
                    <TouchableOpacity key={i} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 6 }} onPress={() => setUrlResults(prev => prev.map((r, j) => j === i ? { ...r, checked: !r.checked } : r))}>
                      <View style={{ width: 20, height: 20, borderRadius: 4, borderWidth: 1.5, borderColor: h.checked ? SCHOOL_THEME : colors.textDisabled, backgroundColor: h.checked ? SCHOOL_THEME : 'transparent', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  <TouchableOpacity style={[styles.modalBtn, { backgroundColor: SCHOOL_THEME, marginTop: 12 }]} onPress={handleSaveUrlImport} disabled={urlResults.filter(h => h.checked).length === 0}>
      ...
      <ActionModal visible={actionModal.visible} title={actionModal.title} subtitle={t('school.holiday')} onEdit={() => { const h = holidays.find(h => h.id === actionModal.id); if (h) { setEditingId(h.id); setForm({ title: h.title, dateFrom: h.dateFrom, dateTo: h.dateTo || '', timeFrom: h.timeFrom || '', timeTo: h.timeTo || '' }); setShowAddModal(true); } setActionModal({ visible: false, id: '', title: '' }); }} onDelete={async () => { await deleteSchoolHoliday(actionModal.id); setActionModal({ visible: false, id: '', title: '' }); loadHolidays(); }} onCancel={() => setActionModal({ visible: false, id: '', title: '' })} />
    </SafeAreaView>
  );
};
