...
import { View, Text, ScrollView, TouchableOpacity, StyleSheet, TextInput, Linking, Modal } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { useTheme } from '../theme/ThemeContext';
import { useTranslation } from 'react-i18next';
import { AppIcon } from '../components/AppIcon';
import { MODULE_COLORS } from '../constants/moduleColors';
import { KindergartenChild, KindergartenYear, KindergartenContact } from '../types';
import { useUserStore } from '../store/userStore';
import { getKindergartenContacts, addKindergartenContact, updateKindergartenContact, deleteKindergartenContact } from '../services/kindergartenService';
...
interface Props {
  navigation: any;
  route: { params: { child: KindergartenChild; selectedYear: KindergartenYear | null; years: KindergartenYear[] } };
}

export const KindergartenContactsScreen: React.FC<Props> = ({ navigation, route }) => {
  ...
  const loadContacts = useCallback(async () => {
    ...
    const data = await getKindergartenContacts(familyId, selectedYear.id);
    setContacts(data);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId) { await updateKindergartenContact(editingId, data); }
      else { await addKindergartenContact(data); }
  ...
  const renderContactCard = (c: KindergartenContact) => (
    <TouchableOpacity key={c.id} style={[styles.contactCard, { backgroundColor: colors.inputBackground }]} onPress={() => navigation.navigate('KindergartenContactDetail', { contact: c, childId: child.id, yearId: selectedYear?.id })} onLongPress={() => setActionModal({ visible: true, id: c.id, title: c.name })}>
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
        <View style={{ flex: 1 }}>
          ...
          {c.teacherType && ['personal', 'contact', 'subject'].includes(c.teacherType) && (
            <View style={[styles.badge, { backgroundColor: c.teacherType === 'personal' ? MODULE_COLORS.kindergartenBg : c.teacherType === 'contact' ? MODULE_COLORS.tripsBg : MODULE_COLORS.birthdaysBg }]}>
        ...
        <View style={{ flexDirection: 'row', gap: 6 }}>
          {c.phone && <TouchableOpacity style={[styles.actionBtn, { backgroundColor: MODULE_COLORS.kindergartenBg }]} onPress={() => Linking.openURL(`tel:${c.phone!.replace(/\s/g, '')}`)}><AppIcon name="phone" size={14} color={MODULE_COLORS.kindergarten} /></TouchableOpacity>}
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top']}>
      <View style={[styles.header, { backgroundColor: colors.surface, borderBottomColor: colors.border }]}>
        ...
        <AppIcon name="contacts" size={24} color={KG_THEME} />
      ...
      <ScrollView style={{ flex: 1, padding: 12 }}>
        ...
        {selectedYear && (
          <TouchableOpacity style={[styles.aiCard, { backgroundColor: MODULE_COLORS.petsBg }]} onPress={() => navigation.navigate('KindergartenAI', { childId: child.id, yearId: selectedYear.id, familyId: familyId || '' })}>
      ...
      <ActionModal
        ...
        onDelete={async () => { await deleteKindergartenContact(actionModal.id); setActionModal({ visible: false, id: '', title: '' }); loadContacts(); }}
