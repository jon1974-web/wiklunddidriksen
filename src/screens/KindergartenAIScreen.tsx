    ...
    if (mode === 'holidays') {
      ...
      try {
        for (const h of selected) {
          await addKindergartenHoliday({ title: h.title, dateFrom: h.dateFrom, dateTo: h.dateTo || h.dateFrom, ...(h.timeFrom ? { timeFrom: h.timeFrom } : {}), ...(h.timeTo ? { timeTo: h.timeTo } : {}), childId, yearId, familyId });
        }
