...
import { PetMedication } from '../types';
import { MODULE_COLORS } from '../constants/moduleColors';
import { formatDate } from '../utils/dateUtils';
import { ActionModal } from '../components/ActionModal';
import { deletePetMedication } from '../services/petService';
...
export const PetMedDetailScreen: React.FC<Props> = ({ navigation, route }) => {
  const { t } = useTranslation();
  const { medication, petName } = route.params as { medication: PetMedication; petName?: string };
  ...
  const isFinished = medication.dateTo && medication.dateTo < new Date().toISOString().slice(0, 10);
  const isActive = medication.dateFrom && (!medication.dateTo || medication.dateTo >= new Date().toISOString().slice(0, 10));

  const canDelete = medication.petId && (medication as any).createdBy === user?.uid || familyRole === 'owner' || familyRole === 'admin';

  const handleDelete = async () => {
    try {
      await deletePetMedication(medication.id);
        ...
        <View style={{ flexDirection: 'row', gap: 8 }}>
          <TouchableOpacity
            ...
            onPress={() => {
              ...
              navigation.navigate('PetSpace', { editId: medication.id, editSection: 'medications', returnToEvents: source === 'events' });
            }}
          >
