...
const SCHOOL_COLOR = MODULE_COLORS.school;

export const SchoolActivityDetailScreen: React.FC<Props> = ({ navigation, route }) => {
  const { t } = useTranslation();
  const { activity, source, child, selectedYear } = route.params as { activity: SchoolActivity; source?: string; child?: any; selectedYear?: any };
  ...
  useEffect(() => {
    ...
    const familyIdVal = activity.familyId;
    if (groupId && familyIdVal) {
      const loadScheduleInfo = async () => {
        try {
          const q = query(collection(db, 'schoolActivities', familyIdVal, 'activities'), where('scheduleGroupId', '==', groupId));
  ...
  }, [activity.scheduleGroupId, activity.familyId]);
  ...
  const handleDelete = async () => {
    try {
      await deleteSchoolActivity(familyId || '', activity.id);
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: MODULE_COLORS.schoolBg }]}>
      ...
      <View style={[styles.card, { borderLeftWidth: 4, borderLeftColor: SCHOOL_COLOR, marginBottom: 10, backgroundColor: colors.surface }]}>
        <View style={{ flexDirection: 'row', gap: 12, alignItems: 'flex-start' }}>
          <View style={styles.calIcon}>
            <View style={[styles.calTopBar, { backgroundColor: SCHOOL_COLOR }]}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <View style={{ flexDirection: 'row', gap: 6, marginTop: 6 }}>
              <View style={[styles.badge, { backgroundColor: SCHOOL_COLOR + '20' }]}>
                <Text style={{ fontSize: 10, fontWeight: '600', color: SCHOOL_COLOR }}>{typeLabel}</Text>
              </View>
      ...
      <View style={[styles.card, { borderLeftWidth: 4, borderLeftColor: SCHOOL_COLOR, backgroundColor: colors.surface }]}>
        <Text style={[styles.sectionLabel, { color: SCHOOL_COLOR }]}>{t('common.details')}</Text>
        {activity.scheduleGroupId && (
          <View style={styles.viewDetailRow}>
            ...
            <Text style={[styles.viewDetailValue, { color: colors.text }]}>
              {scheduleInfo ? `${t('school.recurrenceInfo')} · ${scheduleInfo.weekType === 'odd' ? t('school.recurrenceOdd') : scheduleInfo.weekType === 'even' ? t('school.recurrenceEven') : t('school.recurrenceAll')} · ${scheduleInfo.startDate} – ${scheduleInfo.endDate}` : t('school.recurrenceInfo')}
            </Text>
          </View>
        )}
        {(() => {
        ...
        {activity.note && (
          <View style={{ marginBottom: 12 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 4 }}>
              ...
              <Text style={{ fontSize: 12, fontWeight: '700', color: SCHOOL_COLOR }}>{t('common.note')}</Text>
            </View>
            <View style={{ paddingLeft: 22 }}>
              ...
              {activity.note.length > 60 && (
                <TouchableOpacity onPress={() => setShowFullNote(!showFullNote)}>
                  <Text style={{ fontSize: 12, color: SCHOOL_COLOR, fontWeight: '600', marginTop: 4 }}>
      ...
      {activity.location && mapUrl && (
        <View style={[styles.card, { padding: 0, overflow: 'hidden', borderLeftWidth: 4, borderLeftColor: SCHOOL_COLOR, backgroundColor: colors.surface }]}>
          ...
          <TouchableOpacity onPress={() => Linking.openURL(getGoogleMapsUrl(activity.location!))} style={{ padding: 12, alignItems: 'center' }}>
            <Text style={{ fontSize: 12, color: SCHOOL_COLOR, fontWeight: '600' }}>{t('tips.openGoogleMaps')}</Text>
          </TouchableOpacity>
        </View>
      )}
