...
import { deletePetVaccination } from '../services/petService';
...
export const PetVaccDetailScreen: React.FC<Props> = ({ navigation, route }) => {
  const { t } = useTranslation();
  const { vaccination, petName } = route.params as { vaccination: PetVaccination; petName?: string };
  ...
  const isCompleted = vaccination.status === 'completed' || (vaccination.date && vaccination.date < new Date().toISOString().slice(0, 10));

  const canDelete = vaccination.petId && (vaccination as any).createdBy === user?.uid || familyRole === 'owner' || familyRole === 'admin';

  const handleDelete = async () => {
    try {
      await deletePetVaccination(vaccination.id);
    ...
    <ScrollView style={[styles.container, { backgroundColor: MODULE_COLORS.petsBg }]}>
      ...
      <View style={[styles.card, { borderLeftWidth: 4, borderLeftColor: PET_COLOR, backgroundColor: colors.surface }]}>
        ...
        {vaccination.nextDue && (
          <View style={styles.detailRow}>
            <Text style={[styles.detailLabel, { color: colors.textSecondary }]}>📅</Text>
            <Text style={[styles.detailValue, { color: colors.text }]}>{t('health.nextDose')} {formatDate(vaccination.nextDue)}</Text>
          </View>
        )}
        {vaccination.note && (
        ...
        <View style={{ flexDirection: 'row', gap: 8 }}>
          <TouchableOpacity
            ...
            onPress={() => {
              ...
              navigation.navigate('PetSpace', { editId: vaccination.id, editSection: 'vaccinations', returnToEvents: source === 'events' });
            }}
          >
