  ...
  const searchAddress = (input: string) => {
    ...
    if (Platform.OS === 'web') {
      ...
      autocompleteRef.current.getQueryPredictions(
        { input, types, ...(country ? { componentRestrictions: { country } } : {}) },
      ...
      fetch(
        `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${encodeURIComponent(input)}&key=${GOOGLE_MAPS_API_KEY}&language=no&types=${types.join('|')}${country ? `&components=country:${country}` : ''}`
      )
        .then((res) => res.json())
        .then((data) => {
          if (data.predictions) {
